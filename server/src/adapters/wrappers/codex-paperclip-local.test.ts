...
import {
  CODEX_PAPERCLIP_LOCAL_TYPE,
  createCodexPaperclipLocalAdapter,
  patchCodexPaperclipExecutionContext,
  resolveCodexPaperclipRuntimeState,
} from "./codex-paperclip-local.js";
...
describe("codex paperclip wrapper", () => {
  ...
  it("patches Paperclip env and prompt policy before delegation", async () => {
    ...
    const adapter = createCodexPaperclipLocalAdapter({ base: createBaseAdapter(execute) });
    ...
    expect(adapter.type).toBe(CODEX_PAPERCLIP_LOCAL_TYPE);
  ...
  it("records runtime-state decision on the delegated config", () => {
    const patched = patchCodexPaperclipExecutionContext(createContext({
      config: {
