...
export const CODEX_REMOTE_FAST_MODE_SUPPORTED_MODELS = ["gpt-5.4"] as const;

function normalizeModelId(model: string | null | undefined): string {
  return typeof model === "string" ? model.trim() : "";
}

export function isCodexRemoteKnownModel(model: string | null | undefined): boolean {
  const normalizedModel = normalizeModelId(model);
  if (!normalizedModel) return false;
  return models.some((entry) => entry.id === normalizedModel);
}

export function isCodexRemoteManualModel(model: string | null | undefined): boolean {
  const normalizedModel = normalizeModelId(model);
  return Boolean(normalizedModel) && !isCodexRemoteKnownModel(normalizedModel);
}

export function isCodexRemoteFastModeSupported(model: string | null | undefined): boolean {
  if (isCodexRemoteManualModel(model)) return true;
  const normalizedModel = typeof model === "string" ? model.trim() : "";
  return CODEX_REMOTE_FAST_MODE_SUPPORTED_MODELS.includes(
    normalizedModel as (typeof CODEX_REMOTE_FAST_MODE_SUPPORTED_MODELS)[number],
  );
}
