...
async function extractTarballToDirectory(input: {
  ...
  // on Windows by passing a basename + cwd.
...
export async function prepareSandboxManagedRuntime(input: {
   ...
   * remote workspaces (e.g. codex_remote's Git sandbox that clones the repo
   * itself), where overwriting the remote workspace with a local copy would
   * clobber the clone.
   */
  ...
  await withTempDir("paperclip-sandbox-sync-", async (tempDir) => {
    ...
    if (gitSnapshot) {
      ...
      // upload is skipped (provider-realized remote workspace).
      await input.client.makeDir(runtimeRootDir);
    }
